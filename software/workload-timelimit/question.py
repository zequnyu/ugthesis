...
class Question(QWidget):
    ...
    def setMainLayout(self):
        ...
        self.timerQ.timeout.connect(self.updateLabelQ)
        self.timerQ.start(1000)
        self.remain = QLabel('Time remaining: ' + str(self.countQ), self)
        self.remain.setFont(QFont('SansSerif', 36))
        self.remain.setAlignment(Qt.AlignCenter)

        self.mainLayout.addWidget(btn, 5, 3, Qt.AlignCenter)
        self.mainLayout.addWidget(self.remain, 5, 2, Qt.AlignCenter)
        self.setLayout(self.mainLayout)

    def updateLabelQ(self):
        self.countQ = self.countQ - 1
        self.remain.setText('Time remaining: ' + str(self.countQ))
        if self.countQ <= 0:
            if self.questionNum % 5 == 0:
                self.toRate()
            else:
...
class Timebreak(QWidget):
    ...
    def setMainLayout(self):
        ...
        self.label2 = QLabel('Take a deep breath.\n', self)
